...
const NAV = [
  { hash: "#/dashboard", title: "Сводка", icon: "dashboard", adminOnly: false, group: "main" },
  { hash: "#/my-tasks", title: "Мои задачи", icon: "tasks", adminOnly: false, group: "main" },
  { hash: "#/plans", title: "Анкор-планы", icon: "plans", adminOnly: false, group: "data" },
  { hash: "#/donors", title: "Доноры", icon: "donors", adminOnly: false, group: "data" },
  { hash: "#/stop-list", title: "Стоп-лист", icon: "stop", adminOnly: false, group: "data" },
  { hash: "#/import-export", title: "Импорт / экспорт", icon: "swap", adminOnly: true, group: "admin" },
  { hash: "#/users", title: "Сотрудники", icon: "users", adminOnly: true, group: "admin" },
];
...
const ROUTES = [
  ...
  { pattern: /^#\/donors$/, render: renderDonors },
  { pattern: /^#\/plans$/, render: renderPlans },
  { pattern: /^#\/plans\/(\d+)$/, render: (host, m) => renderPlanDetails(host, parseInt(m[1])) },
  { pattern: /^#\/my-tasks$/, render: renderMyTasks },
  { pattern: /^#\/stop-list$/, render: renderStopList },
  { pattern: /^#\/users$/, render: renderUsers, adminOnly: true },
  { pattern: /^#\/import-export$/, render: renderImportExport, adminOnly: true },
];
...
function buildSidebar(user) {
  ...
  const roleLabel = user.role === "admin" ? "Админ" : "Сотрудник";
  ...
  const groups = {};
  NAV.filter(n => !n.adminOnly || auth.isAdmin()).forEach(n => {
...
async function route() {
  ...
  for (const r of ROUTES) {
    ...
    if (m) {
      if (r.adminOnly && !auth.isAdmin()) {
        host.appendChild(el("div", { class: "empty" },
          el("div", { class: "icon-circle" }, icon("alert", { size: 22 })),
          el("div", { class: "title" }, "Доступ ограничен"),
          el("div", { class: "desc" }, "Этот раздел доступен только администраторам."),
        ));
        matched = true;
        break;
      }
